...
shopRoutes.post('/', async (c) => {
  try {
    ...
    console.log('Shop create user:', JSON.stringify(user));
    ...
    console.error('Shop create error:', String(err).substring(0, 300));
...
shopRoutes.put('/:id', async (c) => {
  try {
    ...
    console.error('Shop update error:', String(err).substring(0, 300));
    return c.json({ message: 'Server error', error: String(err).substring(0, 200) }, 500);
  }
});
